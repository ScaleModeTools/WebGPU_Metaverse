...
function resumeCalibrationSamples(
  ...
  const anchors = gameFoundationConfig.calibration.anchors;
...
export class NinePointCalibrationSession {
  ...
  constructor(
    storedSamples: readonly CalibrationShotSample[] = [],
    captureConfig: CalibrationCaptureConfig = calibrationCaptureConfig
  ) {
    this.#captureConfig = captureConfig;
    this.#storedSamples = [...resumeCalibrationSamples(storedSamples)];
    this.#captureState =
      this.#storedSamples.length >= gameFoundationConfig.calibration.anchors.length
        ? "complete"
        : "waiting-for-hand";
  }
  ...
  get snapshot(): NinePointCalibrationSnapshot {
    const currentAnchor =
      gameFoundationConfig.calibration.anchors[this.#storedSamples.length] ?? null;
    ...
    return Object.freeze({
      ...
      totalAnchorCount: gameFoundationConfig.calibration.anchors.length
    });
  }
  ...
  ingestTrackingSnapshot(
    ...
    const currentAnchor =
      gameFoundationConfig.calibration.anchors[this.#storedSamples.length];
